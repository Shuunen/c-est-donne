  ...
  <div class="grid gap-6">
    ...
    <div class="grid gap-6" :class="{ 'sm:grid-cols-2 md:grid-cols-3': showCards }">
      <div v-for="item, index in state.items" :key="item.id" :style="`animation-delay: ${200 * index}ms;`"
        ...
        <img class=" bg-white object-contain dark:brightness-75 dark:saturate-[1.2]" :src="item.images[0] ?? '/no-visual.svg'"
          :class="{ 'h-64 p-6': showCards, 'min-h-[3rem] max-h-56 w-64 p-4': !showCards }" :alt="`${item.name}image`" />
